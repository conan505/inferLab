#!/usr/bin/env python3
"""Send a sequential circuit-breaker phase and capture gateway/worker state."""
    ...
    payload = json.dumps(
        {
            ...
            "stream": False,
            "messages": [
                {"role": "user", "content": f"circuit phase request {number}"}
            ],
        }
    ).encode("utf-8")
...
def main() -> None:
    ...
    worker_counts = Counter(
        sample["worker"] for sample in samples if sample["worker"] is not None
    )
    latencies = [sample["e2e_ms"] for sample in samples]
    report = {
        "schema": "inferlab.circuit-probe.v0.0.8",
