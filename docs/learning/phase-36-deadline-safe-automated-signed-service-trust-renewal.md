...
| distributor returns with valid transport | a higher signed generation restores service |
...
|---|---|
| template decoding, fingerprint, scheduler and durable state | `service-auth` renewal module |
| persistent process, mTLS HTTP, status and supervision | policy-renewer binary/crate |
