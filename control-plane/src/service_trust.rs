...
pub fn select_service_trust_distribution_mode(
    ...
    match (snapshot_path, distributor_url) {
        ...
        (Some(_), None) if remote_only_configuration_present => Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "INFERLAB_SERVICE_TRUST_CACHE_PATH, INFERLAB_SERVICE_TRUST_REQUEST_TIMEOUT_MS, and INFERLAB_SERVICE_TRUST_MAX_BACKOFF_MS require INFERLAB_SERVICE_TRUST_DISTRIBUTOR_URL",
        )),
        ...
        (None, None) if remote_only_configuration_present || signed_only_configuration_present => {
            Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "service-trust poll, cache, timeout, and backoff configuration require INFERLAB_SERVICE_TRUST_SNAPSHOT_PATH or INFERLAB_SERVICE_TRUST_DISTRIBUTOR_URL",
            ))
        }
...
impl RemoteServiceTrustConfig {
    pub fn new(
        ...
        if distributor_url.scheme() != "http" {
            return Err(invalid_data(
                "service-trust distributor URL scheme must be http; TLS transport is outside the v0.23 trust-artifact boundary",
            ));
        }
...
pub fn bootstrap_signed_service_trust(
    ...
    .map_err(invalid_data)?;
    authorizer.configure_trust_distribution("local-file", "local-file", false, false, false);
...
fn validate_cache(cache: &PersistedServiceTrustCache) -> io::Result<()> {
    ...
    {
        return Err(invalid_data(
            "cached service-trust distributor URL is not a valid credential-free HTTP origin",
        ));
    }
